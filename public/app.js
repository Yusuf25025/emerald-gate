...
function onPlayerStateChange(event) {
    ...
    if (action) {
        socket.emit('video-action', { action: action, time: ytPlayer.getCurrentTime(), source: 'youtube' });
    }
}
...
function joinRoom() {
    ...
    setTimeout(() => { if (!gateShown) showEmeraldGate(); }, 2 * 60 * 1000);
}
...
function loadVideo() {
    ...
    if (currentSource === 'youtube') {
        ...
        if (videoId && ytPlayer) ytPlayer.loadVideoById(videoId);
    }
}
...
function showEmeraldGate() {
    if (isAsleep || gateShown) return;
    gateShown = true;
    const gate = document.getElementById('emeraldGate');
    if (gate) gate.style.display = 'block';
    gateTimeout = setTimeout(() => { startGradualSleep(); }, 60 * 1000);
}
...
function pressEmeraldGate() {
    ...
    setTimeout(() => { if (!gateShown) showEmeraldGate(); }, 15 * 60 * 1000);
}
...
function startGradualSleep() {
    ...
    sleepStepInterval = setInterval(() => {
        ...
        const dimOverlay = document.getElementById('dimOverlay');
        if (dimOverlay) dimOverlay.style.background = 'rgba(0,0,0,' + dimLevel + ')';
        currentVolume = Math.max(1.0 - (steps * 0.2), 0.1);
        setVolume(currentVolume);
        if (status) {
            if (steps < 3) status.textContent = '🌙 Getting sleepy... ' + steps + 'm';
            else if (steps < 5) status.textContent = '🌙 Very sleepy... ' + steps + 'm';
            else status.textContent = '🌙 Almost asleep...';
        }
        if (steps >= maxSteps) {
            clearInterval(sleepStepInterval);
            triggerFullSleep();
...
socket.on('room-state', (state) => { console.log('Room state:', state); });
...
socket.on('video-action', (data) => {
    ...
    if (data.source === 'youtube' && ytPlayer) {
        ...
        const localVideo = document.getElementById('localPlayer');
        if (localVideo) {
            if (Math.abs(localVideo.currentTime - data.time) > 2) localVideo.currentTime = data.time;
...
socket.on('chat-message', (data) => { addMessage(data.text, data.sender); });
...
socket.on('sleep-mode', () => { triggerFullSleep(); });
socket.on('wake-up', () => { wakeUp(); });
socket.on('user-joined', (count) => { addMessage('Someone joined (' + count + ' online)', 'System'); });
socket.on('user-left', (count) => { addMessage('Someone left (' + count + ' online)', 'System'); });
